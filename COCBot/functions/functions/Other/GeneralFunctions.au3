...
Func GetLangText($Key)
	...
	If IsDeclared("cmbLanguage") Then
		...
		$ReturnStr = IniRead(@ScriptDir & "\images\misc\languages\" & $CurrLangSel & ".ini", "general", $Key, "")
		If $ReturnStr = "" Then
			$ReturnStr = IniRead(@ScriptDir & "\images\misc\languages\English.ini", "general", $Key, "")
		EndIf
...
;~ 		EndIf
	Else
		$ReturnStr = IniRead(@ScriptDir & "\images\misc\languages\" & $StartupLanguage & ".ini", "general", $Key, "")
		If $ReturnStr = "" Then
			$ReturnStr = IniRead(@ScriptDir & "\images\misc\languages\English.ini", "general", $Key, "")
		EndIf
;~ 		If IsDeclared("txtLog") And IsDeclared("statLog") Then
...
Func PopulateLanguages()
	$searchfile = FileFindFirstFile(@ScriptDir & "\images\misc\languages\*.ini")
	$txtLang = ""
	While True

...
;~ 						chkUpgrade6()
        ...
        chkUseAllBuilder()
					...
					Case $lblpushbulletenabled
						lblpushbulletenabled()
					Case $btnGitHub
						ShellExecute("https://instagram.com/michaelvallstrom")
					Case $btnCloseBR
...
Func openWebsite()
	ShellExecute("http://www.facebook.com/michael.vallstrom")
EndFunc   ;==>openWebsite
...
Func _btnSaveStrat($Name = "")
	$strPlugInInUse = IniRead($dirStrat & GUICtrlRead($lstStrategies) & ".ini", "plugin", "name", "")
	If $Name <> "" Then
		SetLOG(GetLangText("msgSaved") & $Name, $COLOR_GREEN)
	...
	Else
		...
		If $strNewName <> "" Then
			If StringRegExp($strNewName, "^[A-Za-z0-9_ -\.]+$", 0) Then
				...
				SetLOG(GetLangText("msgSaved") & $strNewName, $COLOR_GREEN)
				_btnRefresh()
